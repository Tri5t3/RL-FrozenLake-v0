...
if __name__ == "__main__":
    ...
    for i in range(EPISODES):
        ...
        while not done:
            ...
            eps_2 = random.uniform(0, 1)
            action_prime = 0
            if eps_2 > EPSILON:
                action_prime = np.argmax(Q_arr[s_prime, :])
            else:
                # action_prime = random.choice(Q_arr[s_prime])
                action_prime = env.action_space.sample()

            Q_arr[state][action] = Q_arr[state][action] + \
                LEARNING_RATE * (reward + DISCOUNT_FACTOR *
                                 Q_arr[s_prime][action_prime]-Q_arr[state][action])

            if done:
                # Q_arr[s_prime][action_prime] = Q_arr[s_prime][action_prime] + \
                #     LEARNING_RATE * ()
                Q_arr[s_prime][action_prime] += 0
